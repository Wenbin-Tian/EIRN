...
class EnhancedModule(nn.Module):  
    ...
    def forward(self, x, rev=False):  
        if not rev:  
            ...
            return x + self.layers(x) * 0.2  # The output is the same for both directions.
...
class ConvDownsampling(nn.Module):
    ...
    def forward(self, x, rev=False):
        if not rev:
            ...
            inv_weights = torch.inverse(self.conv_weights.to('cpu')).to(device)
    ...
    def __init__(self, channel_in=3, channel_out=3, subnet_constructor=None, block_num=[], down_num=2, down_first=False, use_ConvDownsampling=False, down_scale=4):
        ...
        if down_first:
            for i in range(down_num):
                if use_ConvDownsampling:
                    b = EnhancedModule(current_channel)
                    operations.append(b)
